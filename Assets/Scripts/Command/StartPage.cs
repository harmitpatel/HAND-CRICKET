...
public class StartPage : MonoBehaviour 
{
    ...
    private void Positioner()
    {
        ...
        NewGameRect.y = Screen.height / 2 - NewGameRect.height / 2 - 100.0f;
        DemoRect.x = Screen.width / 2 - DemoRect.width / 2;
        DemoRect.y = NewGameRect.y + NewGameRect.height + 50.0f;
        LeaderBoardRect.x = Screen.width / 2 - LeaderBoardRect.width / 2;
        LeaderBoardRect.y = DemoRect.y + DemoRect.height + 50.0f;
        QuitRect.x = Screen.width / 2 - QuitRect.width / 2;
        QuitRect.y = LeaderBoardRect.y + LeaderBoardRect.height + 50.0f;
    }
	...
	void OnGUI()
	{
		...
		if (GUI.Button (NewGameRect, "NEW GAME", (GUIStyle)"")) 
		{
		 ...
		 else if (GUI.Button (DemoRect, "INSTRUCTIONS", (GUIStyle)"Buttons")) 
		{
            m_sceneNumber = 1;
            m_sceneCommanderObj.OpenScene(m_sceneNumber);
		}

		else if (GUI.Button (LeaderBoardRect, "HIGH SCORES", (GUIStyle)"Buttons")) 
		{
            m_sceneNumber = 2;
            m_sceneCommanderObj.OpenScene(m_sceneNumber);
		}

		else if (GUI.Button (QuitRect, "QUIT", (GUIStyle)"Buttons")) 
		{
            m_sceneNumber = 3;
            m_sceneCommanderObj.OpenScene(m_sceneNumber);
		}
